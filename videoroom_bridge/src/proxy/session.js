...
const CooperationPlugin = require('./plugins/cooperation').CooperationPlugin;
...
class Session {
    ...
    constructor(id, janus) {
        ...
        this.startKeepAlive();
        this.cooperationPlugin = new CooperationPlugin({ session: this });
    }
    ...
    event(event) {
        if(this.cooperationPlugin.hasHandle(event.sender)) {
            this.cooperationPlugin.getHandle(event.sender).event(event);
        } else {
