...
const CameraPlugin = require('./plugins/camera').CameraPlugin;
...
class Session {
    ...
    constructor(id, janus) {
        ...
        this.startKeepAlive();
        this.cameraPlugin = new CameraPlugin({ session: this });
    }
    ...
    event(event) {
        if(this.cameraPlugin.hasHandle(event.sender)) {
            this.cameraPlugin.getHandle(event.sender).event(event);
        } else {
